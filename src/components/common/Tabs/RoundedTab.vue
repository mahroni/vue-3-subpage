...
<template>
    <ul class="flex w-fit select-none gap-1 items-center">
        <li v-for="tab in tabs" :key="tab.label" @click="handleTabClick(tab.label)" @keydown.enter="handleTabClick(tab.label)"
            @keydown.space="handleTabClick(tab.label)" :tabindex="0" role="tab" :aria-selected="modelValue === tab.label"
            ...
            <span v-if="tab.icon">
                <component :is="tab.icon" :size="24" :class="modelValue === tab.label ? 'text-[#27B199]' : 'text-text-title'" />
            </span>
            <span :class="modelValue === tab.label ? 'text-[#27B199]' : 'text-text-title'">{{ tab.label }}</span>
        </li>
    </ul>
</template>