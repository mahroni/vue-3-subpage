...
<template>
    <ul class="flex w-fit select-none gap-1 items-center">
        <li v-for="tab in tabs" :key="tab.label" @click="handleTabClick(tab.label)"
            @keydown.enter="handleTabClick(tab.label)" @keydown.space="handleTabClick(tab.label)" :tabindex="0"
            role="tab" :aria-selected="props.modelValue === tab.label" :aria-controls="`tab-panel-${tab.label}`"
