...
import {
  type TiktokChannelList,
  TiktokChannelResponseSchema,
} from '@/types/schemas/channels/tiktok-channel';
...
export const useFetchTiktokChannel = () => {
  const loading = ref(false);
  const meta = ref<MetaPagination>({ ...initMeta });
  const error = ref<Error | null>(null);
  const data = ref<TiktokChannelList>([]);
  ...
  const fetchChannels = async (params?: any) => {
    try {
      ...
      const response = await tiktokApi.get(newParams);

      const validatedResponse = TiktokChannelResponseSchema.parse(response.data);

      data.value = validatedResponse.data;
      meta.value = validatedResponse.meta;
    } catch (err) {
      handleComposableError(err, error, 'Error fetching tiktok channel');
      data.value = [];
      meta.value = { ...initMeta };
    } finally {
      loading.value = false;
