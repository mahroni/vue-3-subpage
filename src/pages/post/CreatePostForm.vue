...
import { useCreatePost } from '@/composables/posts/useCreatePost'; // Adjust path based on your project structure
...
const handleSubmit = async () => {
  ...
  const payload = {
    ...
    tags: postForm.tags ? postForm.tags.split(',').map(tag => tag.trim()).filter(tag => tag) : undefined,
    reactions: {
  ...
  <div>
    <form @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label for="likes" class="block text-sm font-medium text-gray-700 mb-1">
            Likes
          </label>
        ...
        <div>
          <label for="dislikes" class="block text-sm font-medium text-gray-700 mb-1">
            Dislikes
          </label>
