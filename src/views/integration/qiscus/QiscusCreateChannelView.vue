<template>
  <div class="flex flex-col gap-8 px-12 py-8">
    ...
    <!-- Form section -->
    <CreateNewForm @submit="confirmSubmit" @cancel="router.back()" v-if="!isAutoresponderFormOpen" />

    <AutoResponderForm v-if="isAutoresponderFormOpen" @submit="handleSubmit" @cancel="handleCancelAutoResponder" />
  </div>
</template>
...
async function handleSubmit() {
  ...
  await uQiscus.create(channel.value);
